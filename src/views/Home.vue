<script setup>
  import Contact from "@/components/Contact.vue"
</script>

<template>
  <section id="projects" class="pt-32">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1">

      <RouterLink to="earth">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Earth
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/1.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="jul-irreversible">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Jul - Irreversible
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/2.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="wimagatee">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Wimagatée
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/3.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="rita-ora">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Rita Ora "How to be lonely" - Keyframe
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/4.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="ilu-tower">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Ilu Tower
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/5.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="operation-sconce">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Operation Sconse
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/6.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="issou-train">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Issou Train
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/7_2.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="butterfly-88-retreat">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Butterfly-88 Retreat
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/8_2.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="butterfly-88">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Butterfly-88
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/9.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="issou-things">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Issou Things
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/10.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="thabu-island">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Thabu Island
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/11.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="nightmare">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Nightmare
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/12.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="sea-storm">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Sea Storm
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/13.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="explorer">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Explorer
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/14_2.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="bloody-lands">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Bloody Lands
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/15.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="asparth-desert">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Asparth Desert
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/16.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="mountain-pass">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Mountain Pass
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/17.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="deadly-bay">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Deadly Bay
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/18.jpg" />
        </div>
      </RouterLink>

      <RouterLink to="patrol">
        <div class="overflow-hidden rounded relative group h-full w-full">
          <div
            class="rounded z-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-0 text-white flex items-center justify-center">
            <div
              class="text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4  transition duration-300 ease-in-out font-bold uppercase">
              Patrol
            </div>
          </div>
          <img alt="" class="h-full w-full group-hover:scale-110 transition duration-300 ease-in-out"
            src="/projects/preview/19.jpg" />
        </div>
      </RouterLink>

    </div>
  </section>

  <Contact />
</template>
